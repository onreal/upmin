...
export type LogsViewContext = {
  ...
  fetchLogs: (auth: AuthState) => Promise<{ logs: LogSummary[] }>;
  loadDocument: (id: string) => void;
  clearAgentState: () => void;
  openLoggerSettings: () => void;
};

const buildLogSelector = (logs: LogSummary[], currentId?: string) => {
  if (!logs.length) {
    return `
      <div class="field">
...
export const renderLogsView = async ({
  content,
  auth,
  logs,
  setLogs,
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-log-id]").forEach((button) => {
...
export type LogDocumentContext = {
  content: HTMLElement | null;
  auth: AuthState | null;
  doc: RemoteDocument;
  logs: LogSummary[];
...
export const renderLogDocument = ({
  content,
  auth,
  doc,
  logs,
