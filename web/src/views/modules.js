...
export const renderModulePanel = async ({ auth, doc, editor, normalizeModuleList, fetchModuleSettings, findModuleDefinition, ensureModuleSettingsDocument, openModuleSettings, }) => {
    ...
    const moduleSettingsList = await Promise.all(moduleNames.map(async (moduleName) => ({
        name: moduleName,
...
export const renderModulesView = ({ content, modules, navigationPages, clearAgentState, loadDocument, }) => {
    if (!content) {
        return;
    }
    clearAgentState();
