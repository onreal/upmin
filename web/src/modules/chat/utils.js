...
const messageId = (conversationId, createdAt, index) => `${conversationId}:${createdAt ?? index}`;
...
export const renderMessages = (container, messages, options = {}) => {
    if (!messages.length) {
...
export const updateConversationHeader = (titleEl, metaEl, conversation) => {
    if (!conversation) {
...
export const updateChatInputState = (input, send, active) => {
    input.disabled = !active;
    send.disabled = !active;
};
export const renderConversationList = (container, items, activeId, onSelect) => {
    if (!items.length) {
