...
const resolveSchemaKeys = (schema) => {
...
const coerceDataObject = (payload, editor) => {
...
const isSelected = (list, urlKey, url) => {
    if (!Array.isArray(list)) {
        return false;
    }
    return list.some((entry) => isRecord(entry) && entry[urlKey] === url);
};
export const renderGalleryModule = (panel, context) => {
    ...
    const targetKey = typeof outputSettings?.target === "string" && outputSettings.target.trim() !== ""
  ...
  `;
    ...
    let currentVisibility = initialVisibility === "private" || initialVisibility === "public" || initialVisibility === "all"
        ? initialVisibility
        : "all";
    ...
    removeButton?.addEventListener("click", () => {
        ...
        const list = data[targetKey];
        data[targetKey] = list.filter((entry) => !(isRecord(entry) && entry[urlKey] === currentItem?.url));
        editor?.setValue(data);
        closeModal();
        renderGrid();
    });
    deleteButton?.addEventListener("click", async () => {
        ...
        try {
            ...
            if (Array.isArray(data[targetKey])) {
                data[targetKey] = data[targetKey].filter((entry) => !(isRecord(entry) && entry[urlKey] === currentItem?.url));
                editor?.setValue(data);
            }
            closeModal();
            void loadItems();
